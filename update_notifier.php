...
function update_notifier_menu() {  
	...
	if(version_compare($theme_data['Version'], $xml->latest) == -1) {
                update_option('cc-update-available', 1);
		add_dashboard_page( $theme_data['Name'] . 'Theme Updates', $theme_data['Name'] . '<span class="update-plugins count-1"><span class="update-count">!</span></span>', 'administrator', 'colorclean-updates', update_notifier);
	}
        else {
                update_option('cc-update-available', 0);
        }
}  
...
function update_notifier() { 
	$xml = get_latest_theme_version(1); // This tells the function to cache the remote call for 21600 seconds (6 hours)
	$theme_data = get_theme_data(TEMPLATEPATH . '/style.css'); // Get theme data from style.css (current version is what we want) ?>
	...
	<div class="wrap">
        ...
        <div id="instructions" style="max-width: 800px;">
            ...
            <p><strong>Please note:</strong> make a <strong>backup</strong> of the Theme inside your WordPress installation folder <strong>/wp-content/themes/colorclean/</strong></p>
            ...
            <p>Extract the zip's contents, look for the extracted theme folder, and after you have all the new files upload them using FTP to the <strong>/wp-content/themes/colorclean/</strong> folder overwriting the old ones (this is why it's important to backup any changes you've made to the theme files).</p>
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://jonathan-buyck.fr/theme-version/colorclean.xml';
